...
- **`@williamthorsen/change-grammar`**: Renders change records as subject lines and parses them back. It has no dependencies, and a lint block in `eslint.config.ts` keeps its non-test source from reaching outside the package.
...
- **Stale nmr config fails silently**: editing `packages/nmr/src/vitest.ts`, `src/prettier.ts`, or `src/tests.ts` and re-running without rebuilding exercises the previous config with no error. The root conventions guard reaches `tests.ts` through `dist`, so an unrebuilt edit to the check runs the old one green.
