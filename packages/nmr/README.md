...
- **Git subprocesses are isolated.** Every project loads a setup file that points `GIT_CONFIG_GLOBAL` and `GIT_CONFIG_SYSTEM` at the null device, sets `GIT_CONFIG_NOSYSTEM`, and injects `core.excludesFile` through `GIT_CONFIG_COUNT` and its key/value pair, so a test that spawns `git` reads neither the developer's identity, nor a signing config that can block on a passphrase, nor their global ignore rules. The excludes file needs that last step of its own: its path is not config-derived, so git falls back to `$XDG_CONFIG_HOME/git/ignore`, or `~/.config/git/ignore` where that is unset, whatever the config variables say. Overriding `XDG_CONFIG_HOME` would reach it too, and is not used, because other tools spawned by a test read that variable for configuration of their own. The setup file leads each project's `setupFiles`, ahead of every entry the layers add, because a shared setup establishes the environment the rest run in.
