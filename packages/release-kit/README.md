...
1. **Workspace discovery**: reads `pnpm-workspace.yaml` and resolves its `packages` patterns to find workspace directories, applying pnpm's semantics — a `!`-prefixed entry excludes what it matches, wherever it appears in the list. Each directory containing a `package.json` becomes a workspace. If no workspace file is found, the repo is treated as a single-package project; a workspace file whose patterns resolve to no package is an error naming the condition that emptied it.
