...
describe(formatActionHints, () => {
  ...
  it('returns verbose and sync hints when unallowed vulnerabilities exist', () => {
    ...
    expect(output).toContain('  \u{2022} Run `v11y check --prod --verbose` for full report');
    expect(output).toContain('  \u{2022} Run `v11y sync` to add the listed vulnerabilities to the allowlist.');
  });
  ...
  it('returns remove-only hint without verbose hint when only stale entries exist', () => {
    ...
    const output = formatActionHints(result, ['prod']);
    expect(output).toContain('  \u{2022} Run `v11y sync` to remove stale allowlist entries.');
  ...
  it('uses --dev --verbose when only dev scope is audited', () => {
    ...
    expect(output).toContain('Run `v11y check --dev --verbose` for full report');
  });
