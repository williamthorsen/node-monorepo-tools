...
describe(scaffoldConfig, () => {
  ...
  it('creates config file with severityThreshold and $schema', () => {
    ...
    const configPath = path.join(tempDir, '.config', 'v11y-check.config.json');
...
describe(scaffoldFiles, () => {
  ...
  it('writes both the config and the workflow', () => {
    ...
    expect(existsSync(path.join(tempDir, '.config', 'v11y-check.config.json'))).toBe(true);
...
describe(initCommand, () => {
  ...
  it('returns 0 in dry-run mode and does not write either file', () => {
    ...
    expect(existsSync(path.join(tempDir, '.config', 'v11y-check.config.json'))).toBe(false);
